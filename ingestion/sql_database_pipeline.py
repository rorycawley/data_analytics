...
def get_connection_string():
    host = os.getenv('POSTGRES_HOST', 'companies_registry_db')
    port = os.getenv('POSTGRES_PORT', '5432')
    db = os.getenv('POSTGRES_DB', 'companies_registry')
...
#                             destination='duckdb', dataset_name="rfam_data")
...
def load_entire_database() -> None:
    ...
    pipeline = dlt.pipeline(
        pipeline_name="companies_registry_db",
        destination=dlt.destinations.duckdb(
            "/app/data/duckdb/companies_db.duckdb"),
        dataset_name="companies_db_data"
    )
